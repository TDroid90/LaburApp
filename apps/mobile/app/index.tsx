...
const lightColors = { navy: "#063C78", brandNavy: "#063C78", blue: "#078EE9", cyan: "#39BCEB", snow: "#F4FAFD", stone: "#5E7183", orange: "#FF7800", green: "#16825B", line: "#D6E8F2", surface: "#FFFFFF", surfaceSoft: "#F7FAFB", input: "#FBFDFE", avatar: "#DDF0F7", raised: "#EEF5F8", successSurface: "#E7F7F0", successText: "#315F51", warningSurface: "#FFF3E8", warningText: "#9A4700", danger: "#B7452B" };
const darkColors: typeof lightColors = { navy: "#EAF4FC", brandNavy: "#063C78", blue: "#49B2F5", cyan: "#66D0F5", snow: "#07121D", stone: "#AFC2D2", orange: "#FF8A1F", green: "#56D3A1", line: "#29465B", surface: "#10202F", surfaceSoft: "#162A3B", input: "#132738", avatar: "#183A54", raised: "#1B3041", successSurface: "#12372C", successText: "#87E5C1", warningSurface: "#3A291B", warningText: "#FFC078", danger: "#FF8A72" };
...
export default function Home() {
  ...
  function loginDemoAccount(account: SavedSession) {
    ...
    if (account.role === "provider" && !providerProfile) {
      const services = [{ id: "demo-gasista", family: "Instalaciones", service: "Gasista", description: "Reviso instalaciones domiciliarias, detecto fallas y explico las opciones de reparación antes de comenzar.", price: 0, startTime: "", endTime: "" }];
  ...
  async function saveProviderProfile(nextDraft: SavedProviderProfile) {
    ...
    const tariffItems = [{ id: "diagnostic-fee", trade: nextDraft.trade.trim(), label: "Diagnóstico / visita técnica", unit: "visita", unitPrice: diagnosticPrice, enabled: true }];
    ...
    if (supabase && !isDemoSession) {
      ...
      const providerResult = await supabase.from("provider_profiles").upsert({ user_id: user.id, trade_title: nextDraft.trade.trim(), diagnostic_price: diagnosticPrice, bio: nextDraft.bio.trim(), skills_text: nextDraft.skills.trim(), training: nextDraft.training?.trim() || null, certifications: nextDraft.certifications ?? [], zones: [nextDraft.zones], availability: nextDraft.availability.trim(), published: true });
      ...
      const { error: ratesError } = await supabase.from("provider_rate_items").insert({ provider_id: user.id, trade_name: nextDraft.trade.trim(), label: "Diagnóstico / visita técnica", unit: "visita", unit_price: diagnosticPrice, slot_position: 1, active: true });
      if (ratesError) { setProfileBusy(false); return setProfileError(ratesError.message); }
      await supabase.from("provider_service_offers").delete().eq("provider_id", user.id);
      const { error: offersError } = await supabase.from("provider_service_offers").insert(services.map((item, index) => ({ provider_id: user.id, family: item.family, specialization: item.service, description: item.description?.trim(), position: index + 1, active: true })));
      if (offersError) { setProfileBusy(false); return setProfileError(offersError.message); }
    }
    ...
    void enqueueMirrorEvent("Profesionales", { user_name: publishedProfile.displayName, trade: publishedProfile.trade, city: publishedProfile.city, coverage: publishedProfile.zones, services_json: JSON.stringify(services.map((item) => ({ family: item.family, specialization: item.service, description: item.description }))), bio: publishedProfile.bio, source: supabase ? "supabase" : "mobile_demo" });
    void enqueueMirrorEvent("Tarifario", { id: "diagnostic-fee", provider_name: publishedProfile.displayName, trade_name: publishedProfile.trade, label: "Diagnóstico / visita técnica", unit: "visita", unit_price: diagnosticPrice, pricing_mode: "starting_at", active: true });
  ...
  return <SafeAreaView style={styles.safe}>
    <View style={[styles.header, compactHeader && styles.headerCompact]}>
      ...
      <View style={styles.headerActions}>
        <TouchableOpacity accessibilityRole="button" accessibilityLabel={darkMode ? "Usar modo claro" : "Usar modo oscuro"} style={styles.themeButton} onPress={toggleTheme}>
      ...
      {tab === "Inicio" ? <>
        ...
        {!session ? <View style={styles.emptyPanel}>
          ...
          {session.role === "provider" && providerProfile?.published ? <View style={styles.providerPanel}>
            <View style={styles.workCardTop}><Text style={styles.panelEyebrow}>PERFIL DE PRESTADOR</Text><View style={styles.providerHeaderActions}><Text style={styles.publishedBadge}>Publicado</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Editar perfil de prestador" style={styles.editProfileButton} onPress={openProviderProfile}><Text style={styles.editProfileButtonText}>EDITAR</Text></TouchableOpacity></View></View>
    ...
    <AppModal visible={profileModal} onRequestClose={() => setProfileModal(false)}>
      <View style={styles.modalBackdrop}><ProviderProfileForm darkMode={darkMode} key={`${profileDraft.displayName}-${profileModal}`} initialProfile={profileDraft} email={session?.email ?? ""} busy={profileBusy} remoteError={profileError} onCancel={() => setProfileModal(false)} onSubmit={(profile) => void saveProviderProfile(profile)} /></View>
    </AppModal>
    ...
    <AppModal visible={quoteBuilderRequest !== null} onRequestClose={() => setQuoteBuilderRequestId(null)}>
      <View style={styles.modalBackdrop}>{quoteBuilderRequest && <QuoteBuilderForm darkMode={darkMode} request={quoteBuilderRequest} tariffItems={providerProfile?.tariffItems} onCancel={() => setQuoteBuilderRequestId(null)} onSubmit={sendModularQuote} />}</View>
    </AppModal>
...
function createStyles(colors: ThemeColors) {
  ...
  brandRow: { flexDirection: "row", alignItems: "center" }, wordmarkLogo: { width: 176, height: 48 }, wordmarkLogoWide: { width: 210, height: 60 }, headerActions: { flexDirection: "row", alignItems: "center", gap: 8 }, themeButton: { width: 40, height: 40, borderRadius: 12, borderWidth: 1, borderColor: colors.cyan, backgroundColor: "#0B2035", alignItems: "center", justifyContent: "center" }, themeButtonText: { color: "white", fontSize: 19, lineHeight: 22 },
  ...
  content: { padding: 18, paddingBottom: 110 }, hero: { backgroundColor: colors.brandNavy, borderRadius: 24, padding: 22, marginBottom: 24 }, heroTitle: { color: "white", fontSize: 30, lineHeight: 34, fontWeight: "900" }, heroCopy: { color: "#D6E6EE", fontSize: 15, lineHeight: 21, marginTop: 8 }, search: { backgroundColor: colors.surface, color: colors.navy, borderWidth: 1, borderColor: colors.line, borderRadius: 14, minHeight: 52, marginTop: 18, paddingHorizontal: 16, fontSize: 16 }, quickSearches: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 12 }, quickSearch: { minHeight: 26, justifyContent: "center", paddingHorizontal: 9, paddingVertical: 4, borderRadius: 13, borderWidth: 1, borderColor: "rgba(255,255,255,0.28)", backgroundColor: "rgba(255,255,255,0.08)" }, quickSearchActive: { backgroundColor: colors.orange, borderColor: colors.orange }, quickSearchText: { color: "#D8EEFA", fontSize: 11, lineHeight: 14, fontWeight: "700" }, quickSearchTextActive: { color: "white" },
  sectionHeader: { minHeight: 42, marginBottom: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10, zIndex: 20 }, sectionTitle: { flex: 1, fontSize: 20, fontWeight: "800", color: colors.navy }, compactFilters: { flexDirection: "row", alignItems: "center", gap: 7, zIndex: 30 }, dropdownWrap: { position: "relative", zIndex: 31 }, dropdownButton: { height: 36, paddingHorizontal: 11, borderRadius: 10, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surface, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5 }, dropdownButtonText: { color: colors.navy, fontSize: 12, fontWeight: "800" }, dropdownChevron: { color: colors.blue, fontSize: 15, fontWeight: "900", marginTop: -3 }, dropdownMenu: { position: "absolute", top: 41, minWidth: 175, overflow: "hidden", borderRadius: 12, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surface, shadowColor: "#001F3F", shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.36, shadowRadius: 12, elevation: 12 }, sortMenu: { right: 0 }, cityMenu: { right: 0 }, dropdownOption: { minHeight: 40, justifyContent: "center", paddingHorizontal: 13, borderBottomWidth: 1, borderBottomColor: colors.line }, dropdownOptionActive: { backgroundColor: colors.raised }, dropdownOptionText: { color: colors.stone, fontSize: 12, fontWeight: "700" }, dropdownOptionTextActive: { color: colors.navy, fontWeight: "900" },
  card: { backgroundColor: colors.surface, borderRadius: 18, borderWidth: 1, borderColor: colors.line, padding: 15, marginBottom: 12, flexDirection: "row" }, avatar: { width: 52, height: 52, borderRadius: 16, backgroundColor: colors.avatar, alignItems: "center", justifyContent: "center", marginRight: 13 }, avatarText: { color: colors.navy, fontWeight: "900" }, cardBody: { flex: 1 }, row: { flexDirection: "row", justifyContent: "space-between", gap: 8 }, name: { color: colors.navy, fontSize: 17, fontWeight: "800", flex: 1 }, rating: { color: colors.navy, fontWeight: "700" }, trade: { color: colors.blue, fontWeight: "700", marginTop: 2 }, skills: { color: colors.stone, marginTop: 6 }, badge: { color: colors.green, fontSize: 12, fontWeight: "700", marginTop: 7 }, button: { backgroundColor: colors.orange, borderRadius: 12, paddingVertical: 11, alignItems: "center", marginTop: 12 }, buttonText: { color: "white", fontWeight: "800" },
  ...
  sectionPage: { width: "100%", maxWidth: 760, alignSelf: "center" }, pageTitle: { color: colors.navy, fontSize: 28, fontWeight: "900" }, pageCopy: { color: colors.stone, fontSize: 15, lineHeight: 21, marginTop: 5, marginBottom: 18 }, notificationsPanel: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 17, padding: 15, marginBottom: 12 }, notificationHeading: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }, notificationTitle: { color: colors.navy, fontSize: 17, fontWeight: "900", marginTop: 2 }, notificationCount: { minWidth: 30, height: 30, borderRadius: 15, color: "white", backgroundColor: colors.orange, textAlign: "center", lineHeight: 30, fontWeight: "900" }, notificationRow: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 11, borderTopWidth: 1, borderTopColor: colors.line }, notificationCopy: { flex: 1 }, notificationName: { color: colors.navy, fontSize: 13, fontWeight: "900" }, notificationText: { color: colors.stone, fontSize: 11, marginTop: 3 }, notificationAction: { minHeight: 38, borderRadius: 9, backgroundColor: colors.orange, justifyContent: "center", paddingHorizontal: 10 }, notificationActionText: { color: "white", fontSize: 10, fontWeight: "900" }, notificationEmpty: { color: colors.stone, fontSize: 12, paddingVertical: 8 }, simulatorBanner: { backgroundColor: colors.brandNavy, borderRadius: 17, padding: 15, flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 8 }, simulatorCopy: { flex: 1 }, simulatorTitle: { color: "white", fontSize: 15, fontWeight: "900" }, simulatorText: { color: "#CFE5F0", fontSize: 11, lineHeight: 16, marginTop: 3 }, simulatorButton: { minHeight: 40, borderRadius: 10, backgroundColor: colors.orange, alignItems: "center", justifyContent: "center", paddingHorizontal: 12 }, simulatorButtonText: { color: "white", fontSize: 11, fontWeight: "900" }, emptyPanel: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 20, padding: 26, alignItems: "center", marginTop: 8 }, emptyIcon: { fontSize: 48, marginBottom: 12 }, centerCopy: { color: colors.stone, textAlign: "center", lineHeight: 21, marginTop: 8, maxWidth: 430 },
  workCard: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 18, padding: 17, marginTop: 12 }, workCardTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10 }, workStatus: { color: colors.orange, fontSize: 12, fontWeight: "900" }, statusGreen: { color: colors.green }, statusBlue: { color: colors.blue }, statusRed: { color: colors.danger }, workDate: { color: colors.stone, fontSize: 12 }, workProvider: { color: colors.navy, fontSize: 19, fontWeight: "900", marginTop: 12 }, workDescription: { color: colors.stone, lineHeight: 20, marginTop: 10 }, workMeta: { color: colors.navy, fontSize: 13, marginTop: 7 }, nextStep: { backgroundColor: colors.warningSurface, borderRadius: 10, padding: 11, marginTop: 13 }, nextStepText: { color: colors.warningText, fontSize: 12, fontWeight: "700" }, quoteBox: { borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surfaceSoft, borderRadius: 13, padding: 13, marginTop: 13 }, quoteLabel: { color: colors.blue, fontSize: 11, fontWeight: "900" }, quoteAmount: { color: colors.navy, fontSize: 20, fontWeight: "900" }, quoteScope: { color: colors.navy, fontSize: 13, lineHeight: 18, marginTop: 8 }, quoteEta: { color: colors.stone, fontSize: 12, marginTop: 5 }, quoteBreakdown: { borderTopWidth: 1, borderTopColor: colors.line, marginTop: 10, paddingTop: 7 }, quoteLine: { flexDirection: "row", justifyContent: "space-between", gap: 8, paddingVertical: 4 }, quoteLineName: { color: colors.stone, fontSize: 11, flex: 1 }, quoteLinePrice: { color: colors.navy, fontSize: 11, fontWeight: "800" }, quoteNotes: { color: colors.stone, fontSize: 11, lineHeight: 16, marginTop: 7, fontStyle: "italic" }, quoteValidity: { color: colors.blue, fontSize: 10, fontWeight: "800", marginTop: 7 }, paymentBox: { backgroundColor: colors.successSurface, borderRadius: 12, padding: 12, marginTop: 11 }, paymentTitle: { color: colors.green, fontWeight: "900", fontSize: 13 }, paymentText: { color: colors.successText, fontSize: 12, lineHeight: 17, marginTop: 4 }, lastMessage: { color: colors.stone, fontSize: 12, fontStyle: "italic", marginTop: 11 }, actionRow: { flexDirection: "row", gap: 8, marginTop: 12 }, outlineAction: { flex: 1, minHeight: 46, borderWidth: 1, borderColor: colors.blue, borderRadius: 11, alignItems: "center", justifyContent: "center", paddingHorizontal: 8 }, outlineActionText: { color: colors.blue, fontSize: 12, fontWeight: "900", textAlign: "center" }, primaryAction: { flex: 1.25, minHeight: 46, borderRadius: 11, backgroundColor: colors.orange, alignItems: "center", justifyContent: "center", paddingHorizontal: 8 }, primaryActionFull: { minHeight: 46, borderRadius: 11, backgroundColor: colors.orange, alignItems: "center", justifyContent: "center", paddingHorizontal: 10, marginTop: 12 }, primaryActionText: { color: "white", fontSize: 12, fontWeight: "900", textAlign: "center" }, cardLinks: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", gap: 12, marginTop: 14 }, cardLink: { color: colors.blue, fontSize: 12, fontWeight: "800" }, cancelLink: { color: colors.danger, fontSize: 12, fontWeight: "800" }, reviewPublished: { backgroundColor: colors.surfaceSoft, borderRadius: 12, padding: 12, marginTop: 12 }, reviewStars: { color: colors.orange, fontSize: 18, letterSpacing: 2 }, reviewPublishedText: { color: colors.navy, lineHeight: 19, marginTop: 5 }, verifiedReview: { color: colors.green, fontSize: 11, fontWeight: "800", marginTop: 7 },
  accountCard: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 18, padding: 16, flexDirection: "row", alignItems: "center", marginTop: 8 }, profileAvatar: { width: 56, height: 56, borderRadius: 28, backgroundColor: colors.brandNavy, alignItems: "center", justifyContent: "center", marginRight: 13 }, profilePhoto: { width: 56, height: 56, borderRadius: 28, marginRight: 13 }, profileAvatarText: { color: "white", fontSize: 23, fontWeight: "900" }, accountBody: { flex: 1 }, verifiedNameRow: { flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: 7 }, accountName: { color: colors.navy, fontSize: 18, fontWeight: "900" }, verifiedIcon: { width: 19, height: 19, borderRadius: 10, color: "white", backgroundColor: colors.blue, textAlign: "center", lineHeight: 19, fontSize: 11, fontWeight: "900" }, diagnosticBadge: { color: colors.orange, backgroundColor: colors.warningSurface, borderRadius: 11, paddingHorizontal: 8, paddingVertical: 5, fontSize: 9, fontWeight: "900" }, accountEmail: { color: colors.stone, marginTop: 2 }, localBadge: { color: colors.green, fontSize: 11, fontWeight: "800", marginTop: 7 }, followersButton: { alignItems: "center", justifyContent: "center", paddingHorizontal: 8 }, followersCount: { color: colors.navy, fontSize: 17, fontWeight: "900" }, followersLabel: { color: colors.blue, fontSize: 9, fontWeight: "800" }, followersPanel: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 15, padding: 14, marginTop: 10 }, followerRow: { minHeight: 45, flexDirection: "row", alignItems: "center", borderTopWidth: 1, borderTopColor: colors.line }, followerAvatar: { width: 29, height: 29, borderRadius: 15, backgroundColor: colors.avatar, alignItems: "center", justifyContent: "center", marginRight: 9 }, followerInitial: { color: colors.navy, fontWeight: "900" }, followerName: { flex: 1, color: colors.navy, fontSize: 12, fontWeight: "800" }, followingBadge: { color: colors.green, fontSize: 9, fontWeight: "900" }, providerPanel: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 18, padding: 17, marginTop: 14 }, panelEyebrow: { color: colors.stone, fontSize: 11, fontWeight: "900" }, providerHeaderActions: { flexDirection: "row", alignItems: "center", gap: 7 }, publishedBadge: { color: colors.green, backgroundColor: colors.successSurface, borderRadius: 12, paddingHorizontal: 9, paddingVertical: 5, fontSize: 11, fontWeight: "900" }, editProfileButton: { minHeight: 27, justifyContent: "center", borderRadius: 10, borderWidth: 1, borderColor: colors.blue, paddingHorizontal: 10, backgroundColor: colors.raised }, editProfileButtonText: { color: colors.blue, fontSize: 10, fontWeight: "900" }, providerTrade: { color: colors.navy, fontSize: 21, fontWeight: "900", marginTop: 13 }, profileSection: { marginTop: 16, paddingTop: 14, borderTopWidth: 1, borderTopColor: colors.line }, profileSectionText: { color: colors.stone, fontSize: 12, lineHeight: 18, marginTop: 7 }, certificationList: { flexDirection: "row", flexWrap: "wrap", gap: 7, marginTop: 9 }, certificationBadge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 9, paddingVertical: 7, borderRadius: 12, backgroundColor: colors.successSurface, borderWidth: 1, borderColor: colors.green }, certificationIcon: { width: 16, height: 16, borderRadius: 8, color: "white", backgroundColor: colors.green, textAlign: "center", lineHeight: 16, fontSize: 9, fontWeight: "900" }, certificationText: { color: colors.successText, fontSize: 10, fontWeight: "900" }, servicePlanBadge: { color: colors.green, fontSize: 9, fontWeight: "900" }, profileServiceCard: { borderWidth: 1, borderColor: colors.line, borderRadius: 12, padding: 12, marginTop: 9, backgroundColor: colors.surfaceSoft }, profileServiceCopy: { flex: 1 }, profileServiceFamily: { color: colors.blue, fontSize: 9, fontWeight: "900", textTransform: "uppercase", letterSpacing: 0.4 }, profileServiceName: { color: colors.navy, fontSize: 13, fontWeight: "900", marginTop: 4 }, profileServiceDescription: { color: colors.stone, fontSize: 10, lineHeight: 15, marginTop: 5 }, lockedProfileService: { minHeight: 39, justifyContent: "center", borderRadius: 10, backgroundColor: colors.raised, paddingHorizontal: 11, marginTop: 7 }, lockedProfileServiceText: { color: colors.stone, fontSize: 10, fontWeight: "800" }, reviewAverage: { color: colors.orange, fontSize: 12, fontWeight: "900" }, profileReview: { backgroundColor: colors.surfaceSoft, borderRadius: 12, padding: 11, marginTop: 9 }, reviewAuthorRow: { flexDirection: "row", alignItems: "center" }, reviewAvatar: { width: 32, height: 32, borderRadius: 16, backgroundColor: colors.avatar, alignItems: "center", justifyContent: "center", marginRight: 8 }, reviewAvatarText: { color: colors.navy, fontWeight: "900" }, reviewAuthorCopy: { flex: 1 }, reviewAuthor: { color: colors.navy, fontSize: 11, fontWeight: "900" }, reviewDate: { color: colors.stone, fontSize: 8, marginTop: 2 }, reviewStarsSmall: { color: colors.orange, fontSize: 10, letterSpacing: 1 }, reviewComment: { color: colors.stone, fontSize: 11, lineHeight: 16, marginTop: 8 }, providerInvite: { backgroundColor: colors.brandNavy, borderRadius: 20, padding: 20, marginTop: 14 }, providerInviteTitle: { color: "white", fontSize: 20, fontWeight: "900" }, logoutButton: { alignSelf: "center", padding: 14, marginTop: 13 }, logoutText: { color: colors.danger, fontWeight: "800" }, adminMetrics: { flexDirection: "row", flexWrap: "wrap", gap: 10 }, adminMetric: { flexGrow: 1, flexBasis: 150, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 14, padding: 16 }, adminMetricValue: { color: colors.navy, fontSize: 27, fontWeight: "900", marginTop: 6 }, adminLink: { minHeight: 48, borderTopWidth: 1, borderTopColor: colors.line, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, adminLinkText: { color: colors.navy, fontWeight: "800" }, adminLinkArrow: { color: colors.blue, fontSize: 25 },
  toast: { position: "absolute", bottom: 72, left: 18, right: 18, backgroundColor: colors.green, borderRadius: 14, padding: 14, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, toastText: { color: "white", fontWeight: "700", flex: 1 }, toastClose: { color: "white", textDecorationLine: "underline", marginLeft: 12 }, nav: { position: "absolute", bottom: 0, left: 0, right: 0, height: 66, backgroundColor: colors.surface, borderTopWidth: 1, borderTopColor: colors.line, flexDirection: "row" }, navItem: { flex: 1, alignItems: "center", justifyContent: "center" }, navText: { color: colors.stone, fontWeight: "700" }, navActive: { color: colors.orange },
  modalBackdrop: { position: Platform.OS === "web" ? ("fixed" as "absolute") : "absolute", top: 0, right: 0, bottom: 0, left: 0, zIndex: 1000, elevation: 20, backgroundColor: "rgba(0,10,20,0.78)", justifyContent: "flex-end" }, modalCard: { backgroundColor: colors.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 22, paddingBottom: 30, maxHeight: "92%" }, modalClose: { position: "absolute", right: 18, top: 12, zIndex: 2, width: 36, height: 36, borderRadius: 18, backgroundColor: colors.raised, alignItems: "center", justifyContent: "center" }, modalCloseText: { color: colors.navy, fontSize: 27, lineHeight: 29 }, modalTitle: { color: colors.navy, fontSize: 24, fontWeight: "900", marginTop: 4, paddingRight: 36 }, modalCopy: { color: colors.stone, fontSize: 14, lineHeight: 20, marginTop: 6, marginBottom: 14 }, modalInput: { minHeight: 48, borderWidth: 1, borderColor: colors.line, borderRadius: 12, paddingHorizontal: 14, color: colors.navy, fontSize: 15, marginBottom: 10, backgroundColor: colors.input }, multiline: { minHeight: 88, paddingTop: 13, textAlignVertical: "top" }, modalLabel: { color: colors.navy, fontSize: 13, fontWeight: "800", marginTop: 2, marginBottom: 8 }, demoAccounts: { marginBottom: 10 }, demoAccountButton: { minHeight: 50, borderWidth: 1, borderColor: colors.line, borderRadius: 11, paddingHorizontal: 12, marginBottom: 7, flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: colors.surfaceSoft }, demoAccountTitle: { color: colors.navy, fontSize: 12, fontWeight: "900" }, demoAccountEmail: { color: colors.stone, fontSize: 10, marginTop: 2 }, demoAccountArrow: { color: colors.blue, fontSize: 24 }, loginDivider: { flexDirection: "row", alignItems: "center", gap: 8, marginVertical: 8 }, loginDividerLine: { height: 1, flex: 1, backgroundColor: colors.line }, loginDividerText: { color: colors.stone, fontSize: 10 }, twoTradesHint: { color: colors.warningText, backgroundColor: colors.warningSurface, borderRadius: 9, padding: 9, fontSize: 10, lineHeight: 15, marginTop: -2, marginBottom: 9 }, roleRow: { flexDirection: "row", gap: 8, marginBottom: 12 }, roleChoice: { flex: 1, minHeight: 42, borderWidth: 1, borderColor: colors.line, borderRadius: 10, alignItems: "center", justifyContent: "center", paddingHorizontal: 8 }, roleChoiceActive: { borderColor: colors.blue, backgroundColor: colors.raised }, roleChoiceText: { color: colors.stone, fontSize: 12, fontWeight: "800" }, roleChoiceTextActive: { color: colors.navy }, termsRow: { flexDirection: "row", alignItems: "center", marginBottom: 12 }, checkbox: { color: colors.orange, fontSize: 22, marginRight: 7 }, termsText: { color: colors.stone, fontSize: 12, flex: 1 }, modalError: { color: colors.danger, fontSize: 13, fontWeight: "700", marginBottom: 10 }, modalPrimary: { minHeight: 50, borderRadius: 12, backgroundColor: colors.orange, alignItems: "center", justifyContent: "center", marginTop: 4 }, modalPrimaryText: { color: "white", fontSize: 15, fontWeight: "900" }, recoveryLink: { color: colors.navy, fontWeight: "800", textAlign: "center", marginTop: 14 }, modalSwitch: { color: colors.blue, fontWeight: "800", textAlign: "center", marginTop: 15 }, privacyHint: { color: colors.stone, fontSize: 12, lineHeight: 17, marginBottom: 10 },
  buttonDisabled: { opacity: 0.55 },
  stepLabel: { color: colors.orange, fontSize: 11, fontWeight: "900", letterSpacing: 0.7, marginTop: 2 }, progressTrack: { height: 5, borderRadius: 3, backgroundColor: colors.line, marginTop: 9, marginBottom: 15, overflow: "hidden" }, progressFill: { height: 5, borderRadius: 3, backgroundColor: colors.orange }, reviewBox: { backgroundColor: colors.raised, borderRadius: 12, padding: 13, marginBottom: 10 }, reviewTitle: { color: colors.navy, fontWeight: "900", marginBottom: 4 }, reviewText: { color: colors.stone, fontSize: 12, lineHeight: 17 }, wizardActions: { flexDirection: "row", gap: 9, alignItems: "center" }, wizardBack: { minHeight: 50, minWidth: 86, borderRadius: 12, borderWidth: 1, borderColor: colors.blue, alignItems: "center", justifyContent: "center", marginTop: 4 }, wizardBackText: { color: colors.blue, fontWeight: "900" }, wizardPrimary: { flex: 1 },
  messagesList: { maxHeight: 310, minHeight: 150, marginBottom: 11 }, messagesContent: { paddingVertical: 5, gap: 8 }, chatEmpty: { minHeight: 130, alignItems: "center", justifyContent: "center", paddingHorizontal: 20 }, chatEmptyTitle: { color: colors.navy, fontWeight: "900", marginBottom: 6 }, messageBubble: { maxWidth: "88%", borderRadius: 13, padding: 11 }, messageClient: { alignSelf: "flex-end", backgroundColor: colors.avatar }, messageProvider: { alignSelf: "flex-start", backgroundColor: colors.raised }, messageSystem: { alignSelf: "center", maxWidth: "100%", backgroundColor: colors.warningSurface }, messageSender: { color: colors.blue, fontSize: 10, fontWeight: "900", marginBottom: 3 }, messageBody: { color: colors.navy, fontSize: 13, lineHeight: 18 }, chatInput: { minHeight: 68, paddingTop: 12, textAlignVertical: "top" }, starsRow: { flexDirection: "row", justifyContent: "center", gap: 7, marginBottom: 17 }, starChoice: { color: colors.line, fontSize: 38 }, starChoiceActive: { color: colors.orange },
  });
}
