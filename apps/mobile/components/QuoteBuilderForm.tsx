...
export function QuoteBuilderForm({ darkMode, request, tariffItems = emptyTariffItems, onCancel, onDismiss, onSubmit }: { darkMode: boolean; request: SavedRequest; tariffItems?: SavedTariffItem[]; onCancel: () => void; onDismiss: () => void; onSubmit: (quote: Omit<SavedQuote, "version">) => void }) {
  ...
  useEffect(() => {
    ...
    if (request.quote) {
      setDraft({
        ...
        responseText: request.quote.scope || template.responseText,
  ...
  function send() {
    ...
    if (draft.responseText.trim().length < 10) return setError("Escribí una respuesta breve para el cliente.");
    if (containsContactAttempt(`${draft.responseText} ${draft.eta} ${draft.notes} ${draft.items.map((item) => item.label).join(" ")}`, [request.id])) return setError("No está permitido compartir teléfonos de contacto o emails.");
  ...
  return <View style={styles.card}>
    ...
    <Text style={styles.copy}>{request.trade} · ID {request.displayId ?? request.id} · Versión {(request.quote?.version ?? 0) + 1}</Text>
    <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
      <View style={styles.requestSummary}>
        <Text style={styles.requestSummaryEyebrow}>PEDIDO DE {request.clientName?.toLocaleUpperCase("es") ?? "CLIENTE"}</Text>
        <Text style={styles.requestDescription}>{request.description}</Text>
        {!!request.zone && <Text style={styles.requestMeta}>Zona: {request.zone}</Text>}
        {!!request.desiredAt && <Text style={styles.requestMeta}>Cuándo: {request.desiredAt}</Text>}
        {!!request.attachments?.length && <View style={styles.requestPhotos}>{request.attachments.slice(0, 3).map((photo, index) => <Image key={photo.id} accessibilityLabel={`Foto ${index + 1} enviada por el cliente`} source={{ uri: photo.uri }} resizeMode="cover" style={styles.requestPhoto} />)}</View>}
      </View>

      <Text style={styles.label}>Respuesta para el cliente</Text>
      <TextInput multiline value={draft.responseText} onChangeText={(responseText) => setDraft((current) => ({ ...current, responseText }))} placeholder="Explicá brevemente qué incluye tu propuesta" placeholderTextColor="#71818B" style={[styles.input, styles.responseInput]} />

      <Text style={styles.label}>Modalidad de cobro</Text>
      <Text style={styles.exclusiveHint}>Elegí una sola opción. Sólo esa modalidad se enviará.</Text>
      <View style={[styles.modeRow, compact && styles.modeRowCompact]}>{modes.map((mode) => <TouchableOpacity accessibilityRole="radio" accessibilityState={{ checked: draft.pricingMode === mode.value }} key={mode.value} onPress={() => setDraft((current) => ({ ...current, pricingMode: mode.value }))} style={[styles.modeButton, compact && styles.modeButtonCompact, draft.pricingMode === mode.value && styles.modeButtonActive]}><Text style={[styles.modeText, draft.pricingMode === mode.value && styles.modeTextActive]}>{draft.pricingMode === mode.value ? "● " : "○ "}{mode.label}</Text></TouchableOpacity>)}</View>
      <Text style={styles.hint}>{selectedMode.hint}</Text>

      {draft.pricingMode === "itemized" ? <>
        <Text style={styles.label}>Conceptos</Text>
        {!!availableTariff.length && <View style={styles.tariffNotice}><Text style={styles.tariffNoticeTitle}>✓ Cargado desde tu tarifario</Text><Text style={styles.tariffNoticeText}>Elegí conceptos guardados o ajustalos para este trabajo.</Text><View style={styles.savedOptions}>{availableTariff.map((saved) => <TouchableOpacity key={saved.id} onPress={() => setDraft((current) => current.items.some((item) => item.id === saved.id) ? current : { ...current, eta: current.eta.trim() || (saved.estimatedHours ? `${saved.estimatedHours} horas` : current.eta), items: [...current.items, { id: saved.id, label: saved.label, quantity: 1, unit: saved.unit, unitPrice: saved.unitPrice }] })} style={styles.savedOption}><Text style={styles.savedOptionText}>+ {saved.label}{saved.estimatedHours ? ` · ${saved.estimatedHours} h` : ""}</Text></TouchableOpacity>)}</View></View>}
      ...
      <Text style={styles.label}>Observaciones y exclusiones</Text>
    ...
    <View style={[styles.actions, compact && styles.actionsCompact]}><TouchableOpacity accessibilityRole="button" style={styles.cancel} onPress={onCancel}><Text style={styles.cancelText}>Cerrar</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" style={styles.dismiss} onPress={onDismiss}><Text style={styles.dismissText}>Desestimar</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" style={styles.submit} onPress={send}><Text style={styles.submitText}>Confirmar presupuesto</Text></TouchableOpacity></View>
  </View>;
}
