...
export function ProviderProfileForm({ darkMode, initialProfile, email, busy, remoteError, onCancel, onSubmit }: {
  ...
  const [draft, setDraft] = useState<SavedProviderProfile>(() => ({
    ...
    availabilityEnd: initialProfile.availabilityEnd ?? initialProfile.services?.find((item) => item.endTime)?.endTime ?? "18:00",
  }));
  ...
  function submit() {
    ...
    if (!draft.availabilityStart || !draft.availabilityEnd || draft.availabilityStart >= draft.availabilityEnd) return setLocalError("Elegí un horario de disponibilidad válido.");
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.documentPanel}>
        <View style={styles.documentPanelCopy}>
          ...
          <Text style={styles.documentPanelText}>{draft.certifications?.length ? `${draft.certifications.length} certificación${draft.certifications.length === 1 ? "" : "es"} seleccionada${draft.certifications.length === 1 ? "" : "s"}. Se admite un archivo por cada una.` : "Primero agregá una certificación para poder respaldarla."}</Text>
        </View>
        <TouchableOpacity accessibilityRole="button" disabled={!draft.certifications?.length} style={[styles.credentialUpload, !draft.certifications?.length && styles.disabled]} onPress={() => setCredentialModalOpen(true)}><Text style={styles.credentialUploadText}>Cargar documentación</Text></TouchableOpacity>
      </View>

      <Modal visible={credentialModalOpen} transparent animationType="fade" onRequestClose={() => setCredentialModalOpen(false)}>
        <View style={styles.credentialModalBackdrop}>
          <View style={styles.credentialModalCard}>
            <View style={styles.credentialModalHeader}>
              <View style={styles.credentialModalHeading}><Text style={styles.credentialModalEyebrow}>VERIFICACIÓN PRIVADA</Text><Text style={styles.credentialModalTitle}>Comprobantes de certificaciones</Text></View>
              <TouchableOpacity accessibilityRole="button" accessibilityLabel="Cerrar carga de certificaciones" style={styles.credentialModalClose} onPress={() => setCredentialModalOpen(false)}><Text style={styles.credentialModalCloseText}>×</Text></TouchableOpacity>
            </View>
            <ScrollView style={styles.credentialModalScroll} contentContainerStyle={styles.credentialModalContent} keyboardShouldPersistTaps="handled">
                ...
                {draft.certifications?.map((certification) => {
                  ...
                  return <View key={`evidence-${certification}`} style={styles.credentialCard}>
                    <View style={styles.credentialHeader}>
                      <View style={styles.credentialTitleWrap}><Text style={styles.credentialTitle}>{certification}</Text><Text style={styles.credentialPrivacy}>Sólo lo ven el titular y administración.</Text></View>
              ...
              <TouchableOpacity accessibilityRole="button" style={styles.credentialModalDone} onPress={() => setCredentialModalOpen(false)}><Text style={styles.credentialModalDoneText}>Listo</Text></TouchableOpacity>
            </ScrollView>
          </View>
        </View>
      </Modal>
