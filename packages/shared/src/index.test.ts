...
describe("dominio LaburApp", () => {
  ...
  it("detecta contacto evidente y tolera direcciones cortas", () => {
    expect(containsContactAttempt("Escribime al 49 6 5 6 5 4 9 6 5 6 5")).toBe(true);
    expect(containsContactAttempt("Mi teléfono es 2964_555_999")).toBe(true);
    expect(containsContactAttempt("La altura aproximada es 4250")).toBe(false);
    expect(containsContactAttempt("Necesito reparar una canilla")).toBe(false);
  });
  it("permite el identificador del presupuesto dentro del pedido de cambios", () => {
    const id = "scenario-1788712345678-quote";
    expect(containsContactAttempt(`Solicitud de Cambios Presupuesto ${id}\nCambiar el horario`, [id])).toBe(false);
  });
